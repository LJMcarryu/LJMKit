...
#import "XYVerifyCodeView.h"

@interface XYVerifyCodeView ()
...
@implementation XYVerifyCodeView
...
- (void)cleanCode {
    self.textField.text = @"";
    [self tfEditingChanged:self.textField];
}
...
- (void)tfEditingChanged:(UITextField *)textField {
    ...
    if (textField.text.length >= self.itemCount) {
        ...
        // block 传值
        self.verifyCodeBlack(textField.text);
    }
}
